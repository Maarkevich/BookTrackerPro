    ...
    <div class="challenge-card" data-ch-id="${ch.id}">
      ...
      <div class="challenge-info">
        ...
        <div class="challenge-meta">
          ${gt.icon} ${prog.current} / ${prog.target} ${gt.unit}
...
export function renderChallengeDetail(container, challenge, books, callbacks) {
  ...
  container.innerHTML = `
    <button id="chd-back" class="btn-secondary mb-16">← Челленджи</button>
    ...
    <div class="challenge-hero ${challenge.status}">
      ...
      <div class="challenge-hero-actions">
        <button id="chd-edit" class="btn-secondary" style="padding:8px 12px">✏️</button>
        <button id="chd-del" class="btn-danger" style="padding:8px 12px">🗑️</button>
      </div>
    </div>
    ...
    <div class="challenge-progress-hero">
      ...
      <div class="challenge-progress-detail">
        ${gt.icon} ${prog.detail} · ${prog.percent}%
      </div>
    </div>
    ...
    <div class="detail-section">
          ...
          : notes.map((n, i) => `
            <div class="challenge-note">
              ...
              <button data-note-del="${i}" class="icon-btn"
                      style="width:28px;height:28px;font-size:.8rem;flex-shrink:0">🗑️</button>
            </div>
          `).join('')}
      </div>
      <div class="flex gap-8 mt-8">
        ...
        <button id="chd-note-add" class="btn-secondary" style="width:auto;flex-shrink:0">＋</button>
      </div>
    </div>
  `;
  ...
  return `
    <div class="content-list-item" data-chd-book="${b.id}" style="cursor:pointer">
      ...
      <button data-chd-book-del="${b.id}" class="icon-btn"
              ...
              title="Убрать из челленджа">✕</button>
    </div>
  `;
}
  ...
  overlay.innerHTML = `
    <div class="overlay-panel" style="max-height:88dvh">
      <div class="overlay-header">
        <h2>${isEdit ? '✏️ Редактировать челлендж' : '🏆 Новый челлендж'}</h2>
        <button class="icon-btn ch-form-close">✕</button>
      </div>
      <div class="overlay-body">
          ...
          <div class="goal-type-grid">
            ${Object.entries(GOAL_TYPES).map(([key, gt]) => `
              <button class="goal-type-btn ${(c.goalType || 'books') === key ? 'active' : ''}"
                      data-goal="${key}">
                <span class="goal-type-icon">${gt.icon}</span>
        ...
        <button id="ch-f-save" class="btn-primary">💾 Сохранить</button>
      </div>
    </div>
  `;
  ...
  overlay.innerHTML = `
    <div class="overlay-panel" style="max-height:80dvh">
      <div class="overlay-header">
        ...
        <button class="icon-btn ch-books-close">✕</button>
      </div>
      <div class="overlay-body">
        ${available.length === 0 ? `
...
const CHALLENGE_STYLES = `
  ...
  .challenge-meta { font-size:.78rem; color:var(--text-secondary); margin:2px 0 6px; }
  ...
  .goal-type-btn:hover { border-color:var(--border); }
  .goal-type-btn.active {
    border-color:var(--accent);
    background:var(--accent-dim);
    color:var(--accent);
  }
  .goal-type-icon { font-size:1.4rem; }
