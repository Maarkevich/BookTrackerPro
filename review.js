...
//
//    Новое в 3.5.0:
//      — Хром на SVG-иконках (icons.js) вместо эмодзи
//      — Кнопка «Копировать отзыв» в карточке и в форме
...
export function openReviewForm(bookId) {
  ...
  loadBooks().then(books => {
    ...
    title.textContent = `${icon('pen', 18)} Отзыв: ${book.title}`;
...
function renderReviewFormBody(body, book) {
  ...
  function addQuote(text, page) {
    ...
    currentQuotes.push({ text: text, page: page || 0, used: false });
    rerenderQuotes();
  }
