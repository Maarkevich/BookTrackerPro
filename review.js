...
//      — 🐛 Фикс: нативный confirm() заменён на showConfirm() (uikit.js)
//      — 🏗️ esc / showToast импортируются из utils.js (разрыв цикла)
//      — ♿ SVG-звёзды вместо эмодзи в фильтрах и карточках
//      — ♿ Keyboard-навигация рейтинга (← →) в форме
//      — ♿ role="radiogroup" / aria-атрибуты
//
//    Сохранено из 3.7.0:
...
// ═══════════════════════════════════════════════
//  ХЕЛПЕР: ряд SVG-звёзд (заполненные + пустые)
// ═══════════════════════════════════════════════
...
export function renderReviewsTab(container, books, callbacks) {
  ...
  // Точные значения рейтинга: 5, 4, 3, 2, 1 (🆕 SVG-звёзды)
  const filters = [
    { id: 'all',        label: `Все (${withReviews.length})` },
    { id: 'rated5',     label: `${starsRow(5, 11)} 5` },
    { id: 'rated4',     label: `${starsRow(4, 11)} 4` },
    { id: 'rated3',     label: `${starsRow(3, 11)} 3` },
    { id: 'rated2',     label: `${starsRow(2, 11)} 2` },
    { id: 'rated1',     label: `${starsRow(1, 11)} 1` },
    ...
    <div class="review-card" data-book-id="${book.id}">
      <div class="review-header">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div class="review-stars" aria-label="Оценка: ${r.rating || 0} из 5">${starsRow(r.rating || 0, 14)}</div>
        </div>
      </div>
      ${r.pros ? `<div class="review-pros">${icon('check', 12)} ${esc(r.pros)}</div>` : ''}
      ${r.cons ? `<div class="review-cons">${icon('close', 12)} ${esc(r.cons)}</div>` : ''}
      ...
      <div class="flex gap-8 mt-8" style="flex-wrap:wrap">
        ...
        <button data-del-review="${book.id}" class="btn-danger" style="padding:6px 12px;font-size:.78rem" aria-label="Удалить отзыв">${icon('trash', 13)}</button>
      </div>
    </div>
  `;
}
...
function renderReviewFormBody(body, book) {
  ...
  body.innerHTML = `
    ...
    <!-- Оценка (🆕 radiogroup + keyboard) -->
    <div class="form-group">
      <label>${icon('star', 13)} Оценка</label>
      <div class="star-rating" id="rf-stars" role="radiogroup" aria-label="Оценка книги от 1 до 5" tabindex="0">
        ${[1,2,3,4,5].map(i => `<span class="star ${(r.rating || 0) >= i ? 'filled' : ''}" data-star="${i}" role="radio" aria-checked="${(r.rating || 0) === i}">★</span>`).join('')}
      </div>
    ...
    <div class="form-section">
      ...
      <div class="flex gap-8 mt-8">
        <input type="text" id="rf-quote-text" placeholder="Текст цитаты..." style="flex:1" aria-label="Текст цитаты"/>
        <input type="number" id="rf-quote-page" placeholder="Стр." style="width:70px" min="0" aria-label="Страница"/>
        <button id="rf-quote-add" class="btn-secondary" style="width:auto;flex-shrink:0" aria-label="Добавить цитату">${icon('plus', 14)}</button>
      </div>
    ...
    <div class="toggle-row">
      ...
      <div class="toggle ${r.spoilerFree !== false ? 'active' : ''}" id="rf-spoiler" role="switch" aria-checked="${r.spoilerFree !== false}" tabindex="0"></div>
    </div>
  ...
  function paintStars(n) {
    starsEl.querySelectorAll('.star').forEach(s => {
      ...
      s.setAttribute('aria-checked', v === n);
    });
  }
  ...
  spoilerToggle.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleSpoiler.call(spoilerToggle); }
  });

  // Кастомный селект рекомендации (🆕 SVG-иконки)
  attachCustomSelect(body.querySelector('#rf-recommendation'), {
    renderOption: (opt) => {
      const ic = opt.value === 'Да, рекомендую' ? 'check'
               : opt.value === 'Нет, не рекомендую' ? 'close'
               : opt.value === 'С оговорками' ? 'mystery' : 'target';
  ...
  // ── Удаление (🆕 showConfirm вместо нативного confirm) ──
    ...
    <div class="quote-item" style="align-items:center">
      ...
      <div class="flex gap-8" style="flex-shrink:0">
        <button data-quote-used="${index}" class="icon-btn" style="width:28px;height:28px"
          aria-label="${quote.used ? 'Снять пометку' : 'Пометить как использованную'}"
          title="${quote.used ? 'Снять пометку' : 'Пометить как использованную'}">
          ${quote.used ? icon('refresh', 13) : icon('check', 13)}
        </button>
        <button data-quote-copy="${index}" class="icon-btn" style="width:28px;height:28px" aria-label="Копировать цитату" title="Копировать цитату">${icon('copy', 13)}</button>
        <button data-quote-del="${index}" class="icon-btn" style="width:28px;height:28px" aria-label="Удалить цитату" title="Удалить цитату">${icon('trash', 13)}</button>
      </div>
    </div>
  `;
}
...
// ═══════════════════════════════════════════════
//  5. ПОДСКАЗКА К ОЦЕНКЕ (🆕 без эмодзи)
// ═══════════════════════════════════════════════
...
// ═══════════════════════════════════════════════
// Эмодзи в тексте для буфера обмена СОХРАНЕНЫ намеренно:
// это контент для описания видео/поста, а не UI.
export function copyReviewToClipboard(book) {
  ...
  // 🐛 v3.8.4 ФИКС: было lines.join(' ') — отзыв в одну строку.
  // Теперь реальные переносы строк.
