...
function renderReviewCard(book) {
  ...
  return `
    <div class="review-card" data-book-id="${book.id}">
      ...
      ` : ''}
      ${r.recommendation ? `<div class="text-small mt-8">🎯 ${esc(r.recommendation)}</div>` : ''}

      <div class="flex gap-8 mt-8" style="flex-wrap:wrap">
        <button data-edit-review="${book.id}" class="btn-secondary" style="padding:6px 12px;font-size:.78rem">✏️ Изменить</button>
        <button data-copy-review="${book.id}" class="btn-secondary" style="padding:6px 12px;font-size:.78rem">📋 Копировать</button>
        <button data-del-review="${book.id}" class="btn-danger" style="padding:6px 12px;font-size:.78rem">🗑️</button>
      </div>
    </div>
  `;
}
  ...
  return `
    <div class="quote-item" style="align-items:center">
      ...
      <div class="flex gap-8" style="flex-shrink:0">
        <button data-quote-used="${index}" class="icon-btn" style="width:28px;height:28px;font-size:.8rem"
                title="${quote.used ? 'Снять пометку' : 'Пометить как использованную'}">
          ${quote.used ? '↩️' : '✅'}
        </button>
        <button data-quote-copy="${index}" class="icon-btn" style="width:28px;height:28px;font-size:.8rem" title="Копировать цитату">📋</button>
        <button data-quote-del="${index}" class="icon-btn" style="width:28px;height:28px;font-size:.8rem" title="Удалить цитату">🗑️</button>
      </div>
    </div>
  `;
}
