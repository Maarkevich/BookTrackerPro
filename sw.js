...
self.addEventListener('fetch', (event) => {
  ...
  if (url.origin !== self.location.origin) { handleExternalRequest(event, url); return; }
  if (request.mode === 'navigate') { handleNavigation(event, request); return; }
  if (url.pathname.endsWith('version.json')) { handleVersionCheck(event, request); return; }
  if (isOcrRequest(url)) { handleOcrAsset(event, request); return; }
    ...
    caches.open(CACHE_NAME).then((cache) =>
      cache.match(cacheKey).then((cached) => {
        const networkFetch = fetch(request)
          .then((response) => { if (response.ok) cache.put(cacheKey, response.clone()); return response; })
          .catch(() => cached);
    ...
    caches.open(OCR_CACHE_NAME).then((cache) =>
      cache.match(cacheKey).then((cached) => {
        ...
        return fetch(request).then((response) => {
          if (response.ok) { cache.put(cacheKey, response.clone()); console.log(`[SW] OCR закеширован: ${request.url}`); }
          return response;
        });
      })
    )
  ...
  event.respondWith(
    fetch(request, { cache: 'no-store' })
      ...
      .catch(() => caches.match(request).then((cached) =>
        cached || new Response(JSON.stringify({ version: 'unknown', cache: CACHE_NAME }),
...
function handleExternalRequest(event, url) {
  ...
  if (API_ORIGINS.some((o) => hostname.includes(o))) {
    event.respondWith(
      fetch(event.request).catch(() =>
        new Response(JSON.stringify({ error: 'offline', message: 'Нет подключения' }),
          { status: 503, headers: { 'Content-Type': 'application/json' } }))
    );
    return;
  }
  if (CACHEABLE_ORIGINS.some((o) => hostname.includes(o))) {
    event.respondWith(
      caches.open(COVER_CACHE_NAME).then((cache) =>
        cache.match(event.request).then((cached) => {
          if (cached) return cached;
          return fetch(event.request).then((response) => {
            if (response.ok) { cache.put(event.request, response.clone()); trimCoverCacheThrottled(cache); }
            return response;
          });
        })
      )
...
self.addEventListener('message', (event) => {
  ...
  if (data === 'CLEAR_COVER_CACHE') {
    const port = event.ports?.[0];
    caches.delete(COVER_CACHE_NAME).then(() => { if (port) port.postMessage({ type: 'COVER_CACHE_CLEARED' }); });
    return;
  }
});

self.addEventListener('sync', (event) => {
  if (event.tag === 'sync-book-metadata') event.waitUntil(syncBookMetadata());
});
...
self.addEventListener('periodicsync', (event) => {
  if (event.tag === 'btp-update-check') event.waitUntil(self.registration.update().catch(() => {}));
});
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(self.registration.showNotification(data.title || 'BookTrackerPro', options));
});
