...
* @param {object[]} challenges — 🆕 v3.8.2: передаются из app.js явно
*/
...
// 🆕 v3.8.2: уведомляем app.js для pushState (жест «назад»)
...
* @param {object[]} books
* @param {object} settings
* @param {object[]} challenges — 🆕 v3.8.2: передаются явно, без глобала
*/
...
// 🆕 v3.8.2: челленджи из параметра, не из window
...
// 🆕 v3.8.2: добавляем bookId — нужен для открытия карточки контента
const contentByDate = {};
...
// 🆕 v3.8.2: кликабельные карточки → read-only карточка контента
dayEl.innerHTML = `
...
// ═══════════════════════════════════════════════
const STATS_STYLES = `
/* v3.8.2: кликабельный контент в календаре */
