...
export function renderCollectionDetail(container, collection, books, callbacks) {
  ...
  container.innerHTML = `
    <button id="col-back" class="btn-secondary mb-16">← Подборки</button>
  ...
  overlay.innerHTML = `
    <div class="overlay-panel" style="max-height:70dvh">
      <div class="overlay-header">
        <h2>${isEdit ? '✏️ Редактировать подборку' : '📂 Новая подборка'}</h2>
...
export function openBookCollectionsPicker(bookId, books, collections, onDone) {
  ...
  overlay.innerHTML = `
    <div class="overlay-panel" style="max-height:75dvh">
      <div class="overlay-header">
        <h2>📂 Подборки</h2>
        <button class="icon-btn picker-close">✕</button>
      </div>
      <div class="overlay-body">
        ...
        <button id="picker-save" class="btn-primary mt-16">💾 Готово</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);
  ...
  overlay.innerHTML = `
    <div class="overlay-panel" style="max-height:80dvh">
      <div class="overlay-header">
        ...
        <button class="icon-btn add-books-close">✕</button>
      </div>
      <div class="overlay-body">
        ${available.length === 0 ? `
...
export function renderDrawerFilters(books) {
  ...
  return `
    ${section('По жанрам', '📂', genres, 'genre')}
    ${section('По авторам', '📂', authors, 'author')}
    ${section('По издательствам', '📂', publishers, 'publisher')}
  `;
}
