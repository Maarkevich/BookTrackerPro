...
export function renderSeriesDetail(container, seriesName, books, callbacks) {
  ...
  container.innerHTML = `
    <button id="series-back" class="btn-secondary mb-16">← Все серии</button>
  ...
  if (slot.type === 'empty') {
    return `
      <div class="series-slot empty">
        ...
        <div class="series-slot-icon">＋</div>
      </div>
    `;
  }
  ...
  return `
    <div class="series-slot book ${isCurrent ? 'current' : ''}"
      ...
      ${b.coverUrl
        ...
        : `<div class="series-slot-cover placeholder">📕</div>`}
      <div class="series-slot-info">
...
const SERIES_STYLES = `
  ...
  .series-slot-sub { font-size:.78rem; color:var(--text-secondary); margin-top:2px; }
