...
//
//    Новое в 3.8.3:
//      — 🔒 Убран секретный ключ ЛитРес из комментариев
//      — fetchT импортируется из utils.js (retry + backoff)
//        вместо локальной копии без retry
//      — Все API-запросы теперь имеют exponential backoff
...
//
//    📋 Ключи ЛитРес настраиваются в Настройках приложения.
//       Catalit: анонимный доступ работает без ключей.
//       Partner: получите ключи на litres.ru/pages/reader_partner
// ─────────────────────────────────────────────
import { extractBookPreview } from './microlink.js';
import { fetchT } from './utils.js'; // 🆕 v3.8.3: retry + backoff из utils.js
...
/**
 * Очищает ISBN от пробелов, дефисов и приводит к верхнему регистру.
 * @param {string} input
 * @returns {string}
 */
...
/**
 * Проверяет контрольную сумму ISBN-10 или ISBN-13.
 * @param {string} input
 * @returns {boolean}
 */
...
/**
 * Конвертирует ISBN-10 в ISBN-13.
 * @param {string} isbn10
 * @returns {string|null}
 */
...
/**
 * Определяет, является ли ISBN российским (978-5, 979-8).
 * @param {string} isbn
 * @returns {boolean}
 */
...
/**
 * Форматирует ISBN для отображения.
 * @param {string} isbn
 * @returns {string}
 */
...
// ═══════════════════════════════════════════════
//  3. КНИГА ПО ССЫЛКЕ НА МАГАЗИН (Microlink)
// ═══════════════════════════════════════════════
 ...
 *
 * Использует extractBookPreview() из microlink.js,
 ...
 * Каскад: Google Books → Open Library → ЛитРес.
 *
 * @param {string} query — поисковый запрос
 * @param {object|null} litres — ключи ЛитРес
 * @returns {Promise<object[]>} — массив найденных книг
 */
...
/**
 * Получает расширенные метаданные книг по ID из Partner API.
 * Требует partnerId и secretKey (настраиваются в Настройках).
 *
 * @param {string[]} ids — массив ID книг ЛитРес (макс. 10)
...
// 🆕 v3.8.3: локальная fetchT УДАЛЕНА — используется импорт из utils.js
// Старая версия не имела retry, новая имеет exponential backoff
