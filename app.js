...
//
//    Новое в 3.8.3:
//      — 🔒 Убран секретный ключ ЛитРес из дефолтов settings
//      — 🏗️ esc/showToast/trackOverlay/untrackOverlay/formatPrice/
//        convertToDefault импортируются из utils.js (разрыв циклов)
//      — 🏗️ trackOverlay/untrackOverlay реэкспортируются для
//        обратной совместимости с challenges.js / collections.js
//      — 🔒 sanitizeColor() для тегов — защита от CSS injection
//      — ♿ Focus trap в оверлеях (Tab не выходит за пределы модалки)
//      — ♿ Keyboard navigation для extra ratings (← → стрелки)
//      — ♿ prefers-reduced-motion для конфетти
//      — 🐛 window._updateCoverPreview заменён на CustomEvent
//      — 📱 openContentForm передаёт S.settings (defaultPlatform)
//
//    Сохранено из 3.8.0:
//      — Форматы книги, тропы, доп. оценки, герои книги
...
// 🆕 v3.8.3: утилиты из utils.js (разрыв циклических импортов)
import {
...
// 🆕 v3.8.3: реэкспорт для обратной совместимости
// (challenges.js, collections.js, review.js могут импортировать из app.js или utils.js)
...
const S = {
  ...
  tags: [],
  settings: {
    // 🆕 v3.8.3: убран секретный ключ ЛитРес из дефолтов
    lrAppId: '', lrSecret: '',
...
//  🆕 v3.8.3: использует utils.js (consumePoppingState, popTopOverlay,
//  hasOverlays, pushSentinel) вместо локальных переменных
// ═══════════════════════════════════════════════
...
function setupBackGesture() {
  ...
  window.addEventListener('popstate', () => {
    // 🆕 v3.8.3: consumePoppingState из utils.js
    if (consumePoppingState()) return;

    // 1) закрываем верхний оверлей
    // 🆕 v3.8.3: popTopOverlay из utils.js
...
async function init() {
  ...
  // 🆕 v3.8.3: убираем skeleton после первого рендера
...
function renderSearchScopes() {
  DOM.searchScopes.innerHTML = SEARCH_SCOPES.map(s => `
    <button class="scope-chip ${S.searchScope === s.id ? 'active' : ''}" data-scope="${s.id}"
            role="tab" aria-selected="${S.searchScope === s.id}">
      ${s.icon} ${s.label}
    </button>
  `).join('');
...
function renderSearchBookTags() {
  ...
  // 🆕 v3.8.3: sanitizeColor для защиты от CSS injection
  DOM.searchBookTags.innerHTML = S.tags.map(t => {
    const safeColor = sanitizeColor(t.color);
    return `
      <button class="booktag-chip ${S.searchBookTag === t.name ? 'active' : ''}"
              data-booktag="${esc(t.name)}"
              style="color:${safeColor};border-color:${safeColor}55"
              aria-pressed="${S.searchBookTag === t.name}">
        🏷️ ${esc(t.name)}
      </button>`;
  }).join('');
...
function renderSearchResults(results, hasQuery) {
  ...
  if (results.books.length) {
    html += searchSection('📚 Книги', results.books.length, results.books.map(b => `
      <div class="sr-item" data-sr-book="${b.id}" tabindex="0" role="button"
           aria-label="Открыть книгу ${esc(b.title)}">
        ${b.coverUrl ? `<img class="sr-cover" src="${b.coverUrl}" alt="Обложка: ${esc(b.title)}" loading="lazy" referrerpolicy="no-referrer"/>` : '<div class="sr-cover ph" aria-hidden="true">📕</div>'}
        <div class="sr-info">
  ...
  if (results.content.length) {
    html += searchSection('🎬 Контент', results.content.length, results.content.map(c => `
      <div class="sr-item" data-sr-content-book="${c.bookId}" data-sr-content-id="${c.id}"
           tabindex="0" role="button" aria-label="Открыть контент: ${esc(c.title || CONTENT_LABELS[c.type] || c.type)}">
        <div class="sr-icon" aria-hidden="true">${CONTENT_ICONS[c.type] || '🎬'}</div>
        <div class="sr-info">
  ...
  if (results.reviews.length) {
    html += searchSection('✍️ Отзывы', results.reviews.length, results.reviews.map(b => `
      <div class="sr-item" data-sr-book="${b.id}" tabindex="0" role="button"
           aria-label="Открыть книгу ${esc(b.title)} с отзывом">
        <div class="sr-icon" aria-hidden="true">✍️</div>
        <div class="sr-info">
          <div class="sr-title">${esc(b.title)}</div>
  ...
  if (results.quotes.length) {
    html += searchSection('💬 Цитаты', results.quotes.length, results.quotes.map(qt => `
      <div class="sr-item" data-sr-book="${qt.bookId}" tabindex="0" role="button"
           aria-label="Открыть книгу с цитатой">
        <div class="sr-icon" aria-hidden="true">💬</div>
        <div class="sr-info">
  ...
  if (results.collections.length) {
    html += searchSection('📂 Подборки', results.collections.length, results.collections.map(c => `
      <div class="sr-item" data-sr-collection="${c.id}" tabindex="0" role="button"
           aria-label="Открыть подборку ${esc(c.name)}">
  ...
  if (results.challenges.length) {
    html += searchSection('🏆 Челленджи', results.challenges.length, results.challenges.map(c => `
      <div class="sr-item" data-sr-challenge="${c.id}" tabindex="0" role="button"
           aria-label="Открыть челлендж ${esc(c.name)}">
        <div class="sr-icon" aria-hidden="true">${c.emoji || '🏆'}</div>
        <div class="sr-info">
          <div class="sr-title">${esc(c.name)}</div>
  ...
  if (results.series.length) {
    html += searchSection('📚 Серии', results.series.length, results.series.map(s => `
      <div class="sr-item" data-sr-series="${esc(s.name)}" tabindex="0" role="button"
           aria-label="Открыть серию ${esc(s.name)}">
  ...
  if (results.tags.length) {
    html += searchSection('🏷️ Теги', results.tags.length, results.tags.map(t => {
      // 🆕 v3.8.3: sanitizeColor
      const safeColor = sanitizeColor(t.color, 'var(--accent)');
      return `
        <div class="sr-item" data-sr-tag="${esc(t.name)}" tabindex="0" role="button"
             aria-label="Фильтровать книги по тегу ${esc(t.name)}">
          <div class="sr-icon" style="color:${safeColor}" aria-hidden="true">🏷️</div>
          <div class="sr-info">
...
function bindSearchResultEvents(el) {
  ...
  // 🆕 v3.8.3: keyboard support для результатов поиска
  const bindItem = (item, fn) => {
  ...
  el.querySelectorAll('[data-sr-book]').forEach(item => {
    bindItem(item, () => go(() => openBookDetail(item.dataset.srBook)));
  });
  el.querySelectorAll('[data-sr-content-book]').forEach(item => {
    bindItem(item, () => {
      ...
      // 🆕 v3.8.3: передаём S.settings для defaultPlatform
      go(() => openContentForm(contentItem, item.dataset.srContentBook, S.settings));
    });
  });
  el.querySelectorAll('[data-sr-collection]').forEach(item => {
    bindItem(item, () => go(() => { S.openCollection = item.dataset.srCollection; renderTab('collections'); }));
  });
  el.querySelectorAll('[data-sr-challenge]').forEach(item => {
    bindItem(item, () => go(() => { S.openChallenge = item.dataset.srChallenge; renderTab('challenges'); }));
  });
  el.querySelectorAll('[data-sr-series]').forEach(item => {
    bindItem(item, () => go(() => { S.openSeries = item.dataset.srSeries; renderTab('series'); }));
  });
  el.querySelectorAll('[data-sr-tag]').forEach(item => {
    bindItem(item, () => go(() => { S.activeFilter = { type: 'tag', value: item.dataset.srTag }; renderTab('books'); }));
  });
}
...
function renderDrawer() {
  ...
  DOM.drawerCollections.innerHTML = S.collections.map(c => `
    <button class="nav-item sub" data-collection="${c.id}"
            aria-label="Открыть подборку ${esc(c.name)}, ${c.bookIds.length} книг">
  ...
  DOM.drawerSeries.innerHTML = series.length === 0
    ...
    : series.map(s => `
      <button class="nav-item sub" data-series="${esc(s.name)}"
              aria-label="Открыть серию ${esc(s.name)}, ${s.read} из ${s.effectiveTotal} прочитано">
  ...
  DOM.drawerFilters.querySelectorAll('.drawer-filter-toggle').forEach(t => {
    t.addEventListener('click', () => {
      ...
      t.setAttribute('aria-expanded', isOpen); // 🆕 v3.8.3
...
function renderActiveFilters() {
  ...
  el.innerHTML = `
    <span class="active-filter-chip">
      ...
      <button id="clear-filter" aria-label="Сбросить фильтр">✕</button>
    </span>
  `;
...
function renderBooksTab() {
  ...
  mc.innerHTML = `
    <div class="filter-bar no-scrollbar" role="group" aria-label="Фильтр книг по статусу">
      ${filters.map(f => `
        <button class="filter-chip ${S.bookFilter === f.id && !S.activeFilter ? 'active' : ''}"
                data-filter="${f.id}"
                aria-pressed="${S.bookFilter === f.id && !S.activeFilter}">${f.label}</button>
      `).join('')}
    </div>
    <div class="book-list">
...
function renderBookCard(book) {
  ...
  const coverHtml = book.coverUrl
    ? `<img class="book-cover" src="${book.coverUrl}" alt="Обложка: ${esc(book.title)}" loading="lazy" referrerpolicy="no-referrer"/>`
  ...
  const tagsHtml = visible.map(t => {
    ...
    ? `<button class="tag-more-btn" data-id="${book.id}" aria-label="Показать все теги книги ${esc(book.title)}">+${allTags.length - 3}</button>` : '');

  return `
    <div class="book-card" data-id="${book.id}" tabindex="0" role="button"
         aria-label="Открыть книгу ${esc(book.title)}">
      ${coverHtml}
      <div class="book-info">
        ...
        <div class="book-meta">
          <button class="book-badge ${statusClass} status-btn" data-book-id="${book.id}"
                  aria-label="Изменить статус книги ${esc(book.title)}: ${st.label}"
        ...
        ${book.status === 'reading' && book.pageCount > 0 ? `
          <div class="book-progress" role="progressbar" aria-valuenow="${progress}" aria-valuemin="0" aria-valuemax="100"
...
function openStatusDropdown(anchor, bookId) {
  ...
  dd.setAttribute('role', 'listbox');                    // 🆕 v3.8.3
  dd.setAttribute('aria-label', 'Выбор статуса книги');  // 🆕 v3.8.3
  dd.innerHTML = Object.entries(BOOK_STATUSES).map(([key, st]) => `
    <button class="status-dropdown-item ${book.status === key ? 'current' : ''}"
            data-status="${key}" role="option"
            aria-selected="${book.status === key}"
            aria-label="Статус: ${st.label}">
      <span aria-hidden="true">${st.icon}</span> ${st.label}
      ${book.status === key ? '<span style="margin-left:auto" aria-hidden="true">✓</span>' : ''}
    </button>
  `).join('');
...
function openRatingModal(book) {
  ...
  modal.setAttribute('role', 'dialog');       // 🆕 v3.8.3
  modal.setAttribute('aria-modal', 'true');   // 🆕 v3.8.3
  modal.setAttribute('aria-label', isDropped ? 'Книга брошена' : 'Поздравляю, прочитано');
  modal.innerHTML = `
    <div class="rating-modal-panel">
      <div class="rating-modal-emoji" aria-hidden="true">${isDropped ? '📕' : '🎉'}</div>
      ...
      <div class="star-rating" id="rm-stars" role="radiogroup" aria-label="Оценка книги от 1 до 5 звёзд">
        ${[1,2,3,4,5].map(i => `
          <button type="button" class="star" data-star="${i}"
  ...
  stars.querySelectorAll('.star').forEach(star => {
    star.addEventListener('click', async () => {
      ...
      stars.querySelectorAll('.star').forEach(s => {
        ...
        s.setAttribute('aria-checked', v === rating);  // 🆕 v3.8.3
      });
...
function openBookForm(book = null) {
  ...
  DOM.formBody.innerHTML = `
    <div class="form-group">
      ...
      <div class="flex gap-8">
        ...
        <button id="bf-scan" class="btn-secondary" style="width:auto;flex-shrink:0" aria-label="Сканировать ISBN">📷</button>
    ...
    <div class="form-group">
      ...
      <div id="bf-webresults" class="web-search-results" aria-live="polite"></div>
    </div>
    <div class="form-group">
      <label>🔗 Или вставьте ссылку на страницу книги</label>
      <div class="flex gap-8">
        <input type="url" id="bf-page-url" placeholder="https://www.litres.ru/book/..."/>
        <button id="bf-page-extract" class="btn-secondary" style="width:auto;flex-shrink:0" aria-label="Извлечь данные по ссылке">📥</button>
      </div>
      <div class="form-hint">ЛитРес, Book24, Ozon — данные извлекутся автоматически (Microlink)</div>
    </div>
    <div class="divider"></div>
    <div class="form-group">
      <label for="bf-title">Название *</label>
      <input type="text" id="bf-title" value="${esc(b.title || '')}" placeholder="Мастер и Маргарита" required/>
    </div>
    <div class="form-group">
      <label for="bf-author">Автор</label>
      <input type="text" id="bf-author" value="${esc(b.author || '')}" placeholder="Михаил Булгаков"/>
    </div>
    <div class="form-row">
      <div class="form-group"><label for="bf-genre">Жанр</label><input type="text" id="bf-genre" value="${esc(b.genre || '')}" placeholder="Классика"/></div>
      <div class="form-group"><label for="bf-publisher">Издательство</label><input type="text" id="bf-publisher" value="${esc(b.publisher || '')}" placeholder="АСТ"/></div>
    </div>
    <div class="form-row-3">
      <div class="form-group"><label for="bf-year">Год</label><input type="text" id="bf-year" value="${esc(b.publishedDate || '')}" placeholder="2024"/></div>
      <div class="form-group"><label for="bf-pages">Страниц</label><input type="number" id="bf-pages" value="${b.pageCount || ''}" placeholder="480" min="0"/></div>
      <div class="form-group"><label for="bf-age">Возраст</label><input type="text" id="bf-age" value="${esc(b.ageRating || '')}" placeholder="16+"/></div>
    </div>
    <div class="form-group"><label for="bf-desc">Описание</label><textarea id="bf-desc" rows="3" placeholder="Аннотация...">${esc(b.description || '')}</textarea></div>
    <div class="form-group">
      <label>Обложка</label>
    ...
    <div class="form-section">
      <h3>📖 Формат книги</h3>
      <div class="form-group">
        <div class="tags-chips" id="bf-formats" role="group" aria-label="Форматы книги">
          ${Object.entries(BOOK_FORMATS).map(([key, fmt]) => `
            <button class="tag-pick-chip ${selectedFormats.has(key) ? 'active' : ''}" data-format="${key}"
                    role="checkbox" aria-checked="${selectedFormats.has(key)}">
              ${fmt.icon} ${fmt.label}
            </button>
          `).join('')}
        </div>
    ...
    <div class="form-section">
      <h3>📚 Серия</h3>
      <div class="form-group"><label for="bf-series">Название серии</label><input type="text" id="bf-series" value="${esc(b.series || '')}" placeholder="Гарри Поттер" autocomplete="off"/></div>
      <div class="form-row">
        <div class="form-group"><label for="bf-series-num">Номер в серии</label><input type="number" id="bf-series-num" value="${b.seriesNumber || ''}" min="1" placeholder="1"/></div>
        <div class="form-group"><label for="bf-series-total">Всего книг в серии</label><input type="number" id="bf-series-total" value="${b.seriesTotal || ''}" min="1" placeholder="7"/></div>
      </div>
    </div>
    <div class="form-section">
      <h3>💰 Цена</h3>
      <div class="form-row">
        <div class="form-group"><label for="bf-price">Цена</label><input type="number" id="bf-price" value="${b.price?.amount || ''}" min="0" placeholder="599"/></div>
        <div class="form-group"><label for="bf-currency">Валюта</label>
    ...
    <div class="form-section">
      <h3>📖 Статус</h3>
      <div class="form-group"><label for="bf-status">Статус</label>
        <select id="bf-status">
          ${Object.entries(BOOK_STATUSES).map(([k, st]) => `<option value="${k}" ${(b.status || 'wishlist') === k ? 'selected' : ''}>${st.icon} ${st.label}</option>`).join('')}
        </select>
      </div>
      <div class="form-row">
        <div class="form-group"><label for="bf-page">Текущая страница</label><input type="number" id="bf-page" value="${b.currentPage || 0}" min="0"/></div>
        <div class="form-group"><label for="bf-rating">Оценка ⭐ (1–5)</label><input type="number" id="bf-rating" value="${b.rating || b.review?.rating || 0}" min="0" max="5"/></div>
      </div>
    </div>
    <div class="form-section">
      ...
      <div class="form-row-2">
        <div class="form-group">
          ...
          <div class="extra-rating-row" data-rating-id="bf-pepper" data-max="5" data-value="${b.pepperRating || 0}"
               role="radiogroup" aria-label="Оценка горячести от 0 до 5" tabindex="0">
            ${[1,2,3,4,5].map(i => `<span class="extra-star${(b.pepperRating || 0) >= i ? ' filled' : ''}" data-val="${i}" role="radio" aria-checked="${(b.pepperRating || 0) === i}">🌶️</span>`).join('')}
        ...
        <div class="form-group">
          ...
          <div class="extra-rating-row" data-rating-id="bf-tear" data-max="5" data-value="${b.tearRating || 0}"
               role="radiogroup" aria-label="Оценка слёз от 0 до 5" tabindex="0">
            ${[1,2,3,4,5].map(i => `<span class="extra-star${(b.tearRating || 0) >= i ? ' filled' : ''}" data-val="${i}" role="radio" aria-checked="${(b.tearRating || 0) === i}">💧</span>`).join('')}
        ...
        <div class="form-group">
          ...
          <div class="extra-rating-row" data-rating-id="bf-intrigue" data-max="5" data-value="${b.intrigueRating || 0}"
               role="radiogroup" aria-label="Оценка интриги от 0 до 5" tabindex="0">
            ${[1,2,3,4,5].map(i => `<span class="extra-star${(b.intrigueRating || 0) >= i ? ' filled' : ''}" data-val="${i}" role="radio" aria-checked="${(b.intrigueRating || 0) === i}">❓</span>`).join('')}
        ...
        <div class="form-group">
          ...
          <div class="extra-rating-row" data-rating-id="bf-horror" data-max="5" data-value="${b.horrorRating || 0}"
               role="radiogroup" aria-label="Оценка жути от 0 до 5" tabindex="0">
            ${[1,2,3,4,5].map(i => `<span class="extra-star${(b.horrorRating || 0) >= i ? ' filled' : ''}" data-val="${i}" role="radio" aria-checked="${(b.horrorRating || 0) === i}">🤦</span>`).join('')}
      ...
      <div id="bf-characters-list">
        ${(b.characters || []).map((ch, i) => `
          <div class="character-row" data-char-idx="${i}">
            ...
            <button type="button" class="icon-btn char-del" title="Удалить" aria-label="Удалить персонажа">✕</button>
          </div>`).join('')}
      </div>
    ...
    <div class="form-section">
      ...
      <div class="form-group">
        ...
        <div id="bf-tags-chips" class="tags-chips" role="group" aria-label="Выбор тегов">
          ${S.tags.length === 0
            ...
            : S.tags.map(t => {
                ...
                return `
                  <button class="tag-pick-chip ${selectedTags.has(t.name) ? 'active' : ''}"
                          data-tag="${esc(t.name)}"
                          style="color:${safeColor};border-color:${safeColor}55"
                          role="checkbox" aria-checked="${selectedTags.has(t.name)}">
                    ${esc(t.name)}
                  </button>`;
              }).join('')}
        </div>
      </div>
      <div class="form-group"><label for="bf-tags">Добавить новые (через запятую)</label>
    ...
    <div class="form-section">
      <h3>💫 Тропы</h3>
      <div class="form-group">
        <label for="bf-tropes">Тропы книги (через запятую)</label>
    ...
    <div class="form-section">
      ...
      <div class="toggle-row">
        ...
        <div class="toggle ${b.isPR ? 'active' : ''}" id="bf-pr-toggle" role="switch" aria-checked="${b.isPR || false}" tabindex="0"></div>
      </div>
      <div id="bf-pr-fields" class="${b.isPR ? '' : 'hidden'}">
        <div class="form-group"><label for="bf-received-from">От кого</label><input type="text" id="bf-received-from" value="${esc(b.receivedFrom || '')}" placeholder="Издательство ЭКСМО"/></div>
        <div class="form-group"><label for="bf-received-date">Дата получения</label><input type="date" id="bf-received-date" value="${b.receivedDate || ''}"/></div>
      </div>
    </div>
    <div class="form-section">
      ...
      <div class="toggle-row">
        ...
        <div class="toggle ${b.jointReading?.active ? 'active' : ''}" id="bf-joint-toggle" role="switch" aria-checked="${b.jointReading?.active || false}" tabindex="0"></div>
      </div>
      <div id="bf-joint-fields" class="${b.jointReading?.active ? '' : 'hidden'}">
        <div class="form-group"><label for="bf-joint-people">Участники (через запятую)</label><input type="text" id="bf-joint-people" value="${esc((b.jointReading?.participants || []).join(', '))}" placeholder="Аня, Маша, Катя"/></div>
        <div class="form-group"><label for="bf-joint-chat">Ссылка на чат</label><input type="url" id="bf-joint-chat" value="${esc(b.jointReading?.chatLink || '')}" placeholder="https://t.me/+..."/></div>
        <div class="form-group"><label for="bf-joint-notes">Заметки</label><input type="text" id="bf-joint-notes" value="${esc(b.jointReading?.notes || '')}" placeholder="Читаем по 3 главы в день"/></div>
      </div>
    </div>
    <div class="form-section">
      <h3>📝 Заметки</h3>
  ...
  // 🆕 v3.8.3: CustomEvent вместо window._updateCoverPreview
  ...
  // 🆕 v3.8.3: слушаем событие cover-url-changed вместо window._updateCoverPreview
  document.addEventListener('cover-url-changed', updateCoverPreview);
  ...
  fb.querySelectorAll('#bf-formats .tag-pick-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      chip.setAttribute('aria-checked', selectedFormats.has(f)); // 🆕 v3.8.3
    });
  });
  ...
  fb.querySelectorAll('#bf-tags-chips .tag-pick-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      chip.classList.toggle('active');
      chip.setAttribute('aria-checked', selectedTags.has(t)); // 🆕 v3.8.3
    });
  });
  ...
  // Toggles с keyboard support
  const toggleHandler = function(fieldsSelector) {
    return function() {
      this.classList.toggle('active');
      this.setAttribute('aria-checked', this.classList.contains('active'));
      fb.querySelector(fieldsSelector).classList.toggle('hidden');
    };
  };
  const prToggleFn = toggleHandler('#bf-pr-fields');
  const jointToggleFn = toggleHandler('#bf-joint-fields');

  fb.querySelector('#bf-pr-toggle').addEventListener('click', prToggleFn);
  fb.querySelector('#bf-joint-toggle').addEventListener('click', jointToggleFn);

  // 🆕 v3.8.3: keyboard support для toggles
  fb.querySelector('#bf-pr-toggle').addEventListener('keydown', (e) => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); prToggleFn.call(fb.querySelector('#bf-pr-toggle')); }
  });
  fb.querySelector('#bf-joint-toggle').addEventListener('keydown', (e) => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); jointToggleFn.call(fb.querySelector('#bf-joint-toggle')); }
  });

  // Extra emoji ratings interaction + 🆕 v3.8.3: keyboard navigation
  fb.querySelectorAll('.extra-rating-row').forEach(row => {
    ...
    function updateStars(val) {
      stars.forEach((s, i) => {
        ...
        s.setAttribute('aria-checked', i === val - 1);
      });
    }
    ...
    // 🆕 v3.8.3: keyboard navigation для extra ratings
...
function openMicrolinkPreview(result, fb) {
  ...
  overlay.setAttribute('aria-label', 'Предпросмотр данных книги');
  overlay.innerHTML = `
    <div class="overlay-panel" style="max-height:88dvh">
      <div class="overlay-header">
        <h2>📥 Предпросмотр данных</h2>
        <button class="icon-btn ml-close" aria-label="Закрыть предпросмотр">✕</button>
      </div>
        ...
        <div id="ml-fields">
          ${(fields || []).map(f => `
            <div class="ml-field" data-field-id="${f.id}">
              ...
              <select class="ml-target" data-field-id="${f.id}" aria-label="Назначение поля ${esc(f.key)}">
  ...
  const close = () => {
    ...
    document.removeEventListener('cover-url-changed', noopFn);
    const anyOpen = [...document.querySelectorAll('.overlay')].some(o => !o.classList.contains('hidden'));
    if (!anyOpen) document.body.style.overflow = '';
  };

  // 🆕 v3.8.3: noop для корректного removeEventListener
  const noopFn = () => {};
  ...
  overlay.querySelector('#ml-apply').addEventListener('click', () => {
    ...
    // 🆕 v3.8.3: CustomEvent вместо window._updateCoverPreview
    document.dispatchEvent(new CustomEvent('cover-url-changed'));
    close();
...
async function handleWebSearch() {
  ...
  resultsEl.innerHTML = results.map((r, i) => {
    ...
    return `
      <div class="web-result" data-idx="${i}" tabindex="0" role="button"
           aria-label="Выбрать книгу ${esc(r.title)}">
        ${r.cover ? `<img class="web-result-cover" src="${r.cover}" alt="Обложка: ${esc(r.title)}" loading="lazy" referrerpolicy="no-referrer"/>` : '<div class="web-result-cover" style="display:flex;align-items:center;justify-content:center" aria-hidden="true">📕</div>'}
  ...
  resultsEl.querySelectorAll('.web-result').forEach(el => {
    ...
    el.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selectResult(); }
    });
  });
}
...
function openBookDetail(bookId) {
  ...
  DOM.detailBody.innerHTML = `
    <div class="detail-hero">
      ${book.coverUrl
        ? `<img class="detail-cover" id="detail-cover-img" src="${book.coverUrl}" alt="Обложка: ${esc(book.title)}" style="cursor:zoom-in" referrerpolicy="no-referrer"/>`
        : `<div class="detail-cover-placeholder" id="detail-cover-ph" style="cursor:pointer" title="Добавить обложку" role="button" tabindex="0" aria-label="Добавить обложку для книги ${esc(book.title)}">${icon('bookClosed', 52)}<span class="cover-ph-title">${esc(book.title)}</span></div>`}
      <div style="flex:1;min-width:0">
        <div class="detail-title">${esc(book.title)}</div>
        <div class="detail-author">${esc(book.author)}</div>
        <div class="book-meta">
          <button class="book-badge badge-status status-btn" data-book-id="${book.id}"
                  aria-label="Изменить статус книги: ${st.label}" aria-haspopup="listbox">${st.icon} ${st.label} ▾</button>
    ...
    ${book.status === 'reading' && book.pageCount > 0 ? `
      <div class="reading-progress">
        <div class="reading-progress-bar" role="progressbar" aria-valuenow="${progress}" aria-valuemin="0" aria-valuemax="100"
             aria-label="Прогресс чтения: ${progress}%">
    ...
    <div class="detail-section">
      ...
      ${contentItems.length === 0 ? '<div class="text-muted text-small">Пока нет контента</div>'
        : contentItems.map(c => `
          <div class="content-list-item content-clickable" data-content-id="${c.id}"
               title="Открыть контент" tabindex="0" role="button"
               aria-label="Открыть контент: ${esc(c.title || CONTENT_LABELS[c.type] || c.type)}">
            <span class="content-list-icon" aria-hidden="true">${CONTENT_ICONS[c.type] || '🎬'}</span>
    ...
    <div class="detail-section">
      ...
      <div class="flex gap-8 mt-8">
        <input type="text" id="dq-text" placeholder="Новая цитата..." style="flex:1" aria-label="Текст новой цитаты"/>
        <input type="number" id="dq-page" placeholder="Стр." style="width:70px" min="0" aria-label="Страница цитаты"/>
        <button id="dq-add" class="btn-secondary" style="width:auto;flex-shrink:0" aria-label="Добавить цитату">＋</button>
      </div>
    ...
    ${(book.pepperRating > 0 || book.tearRating > 0 || book.intrigueRating > 0 || book.horrorRating > 0) ? `
      <div class="detail-section">
        ...
        <div class="extra-ratings-display">
          ${book.pepperRating > 0 ? `<div class="extra-rating-item"><span class="extra-label">🌶️ Горячесть</span><span class="extra-dots" aria-label="${book.pepperRating} из 5">${'🌶️'.repeat(book.pepperRating)}${'·'.repeat(5-book.pepperRating)}</span></div>` : ''}
          ${book.tearRating > 0 ? `<div class="extra-rating-item"><span class="extra-label">💧 Слёзы</span><span class="extra-dots" aria-label="${book.tearRating} из 5">${'💧'.repeat(book.tearRating)}${'·'.repeat(5-book.tearRating)}</span></div>` : ''}
          ${book.intrigueRating > 0 ? `<div class="extra-rating-item"><span class="extra-label">❓ Интрига</span><span class="extra-dots" aria-label="${book.intrigueRating} из 5">${'❓'.repeat(book.intrigueRating)}${'·'.repeat(5-book.intrigueRating)}</span></div>` : ''}
          ${book.horrorRating > 0 ? `<div class="extra-rating-item"><span class="extra-label">🤦 Жуть</span><span class="extra-dots" aria-label="${book.horrorRating} из 5">${'🤦'.repeat(book.horrorRating)}${'·'.repeat(5-book.horrorRating)}</span></div>` : ''}
        </div>
      </div>
    ` : ''}
        ...
        <div class="characters-list">
          ${(book.characters || []).map(ch => `
            <div class="character-item">
              ...
              <span class="character-stars" aria-label="Оценка персонажа: ${ch.rating || 0} из 5">${'⭐'.repeat(ch.rating || 0)}${'☆'.repeat(5-(ch.rating||0))}</span>
            </div>`).join('')}
        </div>
      </div>
    ` : ''}
    ...
    <div class="detail-section">
      ...
      ${review.text || review.rating > 0 ? `
        <div class="review-stars" aria-label="Оценка: ${review.rating || 0} из 5">${'⭐'.repeat(review.rating || 0)}${'☆'.repeat(5 - (review.rating || 0))}</div>
    ...
    <div class="btn-group mt-16">
      ...
      <button id="detail-delete" class="btn-danger" aria-label="Удалить книгу ${esc(book.title)}">🗑️</button>
    </div>
  `;
  ...
  if (coverPh) {
    coverPh.addEventListener('click', () => openCoverViewer(book));
    // 🆕 v3.8.3: keyboard support
    coverPh.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openCoverViewer(book); }
    });
  }
  ...
  // 🆕 v3.8.3: передаём S.settings для defaultPlatform
  db.querySelector('#detail-add-content').addEventListener('click', () => {
    closeOverlay(DOM.detailOverlay);
    openContentForm(null, book.id, S.settings);
  });
...
function showDayContent(dateStr) {
  ...
  ov.setAttribute('role', 'dialog');          // 🆕 v3.8.3
  ov.setAttribute('aria-modal', 'true');      // 🆕 v3.8.3
  ov.setAttribute('aria-label', `Контент на ${dateStr}`);
  ov.innerHTML = `
    <div class="overlay-panel" style="max-width:420px">
      <div class="overlay-header">
        <h2>${icon('calendar', 18)} ${esc(dateStr)}</h2>
        <button class="icon-btn day-ov-close" aria-label="Закрыть">${icon('close', 18)}</button>
      </div>
      <div class="overlay-body">
        ${dayContent.map(c => `
          <div class="content-list-item content-clickable"
               ...
               aria-label="Открыть контент: ${esc(c.title || CONTENT_LABELS[c.type] || c.type)}">
            <span class="content-list-icon" aria-hidden="true">${icon(CONTENT_TYPE_ICONS[c.type] || 'film', 20)}</span>
            <div class="content-list-info">
              <div class="content-list-title">${esc(c.title || CONTENT_LABELS[c.type] || c.type)}</div>
...
function renderSettingsTab() {
  ...
  mc.innerHTML = `
    <div class="settings-section">
      ...
      <div class="toggle-row">
        ...
        <div class="toggle ${s.bloggerMode ? 'active' : ''}" id="set-blogger"
             role="switch" aria-checked="${s.bloggerMode}" tabindex="0"></div>
      </div>
    </div>
    ...
    <div class="settings-section">
      <h3>💰 Цена и валюта</h3>
      <div class="form-group">
        <label for="set-currency">Валюта по умолчанию</label>
        <select id="set-currency">
          ${Object.entries(CURRENCIES).map(([k, c]) => `<option value="${k}" ${s.defaultCurrency === k ? 'selected' : ''}>${c.symbol} ${c.name}</option>`).join('')}
        </select>
      </div>
      <div class="toggle-row"><span class="toggle-label">Показывать цену в карточках</span><div class="toggle ${s.showPriceInCards ? 'active' : ''}" id="set-price-cards" role="switch" aria-checked="${s.showPriceInCards}" tabindex="0"></div></div>
      <div class="toggle-row"><span class="toggle-label">Показывать цену на странице книги</span><div class="toggle ${s.showPriceInDetail ? 'active' : ''}" id="set-price-detail" role="switch" aria-checked="${s.showPriceInDetail}" tabindex="0"></div></div>
      <div class="toggle-row"><span class="toggle-label">Показывать цену в статистике</span><div class="toggle ${s.showPriceInStats ? 'active' : ''}" id="set-price-stats" role="switch" aria-checked="${s.showPriceInStats}" tabindex="0"></div></div>
    ...
    <div class="settings-section">
      ...
      <div class="flex gap-8 mb-16">
        <input type="text" id="set-tag-new" placeholder="Новый тег..." style="flex:1" aria-label="Название нового тега"/>
        <input type="color" id="set-tag-new-color" value="#e8a33d" style="width:44px;height:36px;padding:2px;border-radius:8px;cursor:pointer" aria-label="Цвет нового тега"/>
        <button id="set-tag-add" class="btn-secondary" style="width:auto;flex-shrink:0" aria-label="Добавить тег">＋</button>
      </div>
      <div id="set-tags-list">
        ${S.tags.map(t => `
          <div class="flex items-center gap-8 mb-8">
            <input type="color" data-tag-color="${esc(t.name)}" value="${t.color || '#e8a33d'}" style="width:44px;height:36px;padding:2px;border-radius:8px;cursor:pointer" aria-label="Цвет тега ${esc(t.name)}"/>
            <span class="flex-1 text-small">${esc(t.name)}</span>
            <button data-tag-del="${esc(t.name)}" class="icon-btn" style="width:30px;height:30px;font-size:.8rem" aria-label="Удалить тег ${esc(t.name)}">🗑️</button>
          </div>
    ...
    <div class="settings-section">
      ...
      <div class="form-group">
        <label for="set-microlink-key">API-ключ (если есть Pro)</label>
      ...
      <span id="set-microlink-status" class="status-text" aria-live="polite"></span>
    ...
    <div class="settings-section">
      ...
      <p class="hint">
        ...
        1. <a href="https://www.litres.ru/pages/reader_partner/" target="_blank" rel="noopener">litres.ru/pages/reader_partner</a><br/>
        2. Напишите на <a href="mailto:partners@litres.ru">partners@litres.ru</a>
      </p>
      <details>
        <summary>Catalit API (поиск по ISBN и названию)</summary>
        <div class="form-group mt-8"><label for="set-lr-appid">App ID</label><input type="text" id="set-lr-appid" value="${esc(s.lrAppId || '')}" placeholder="Ваш App ID"/></div>
        <div class="form-group"><label for="set-lr-secret">Secret Key</label><input type="password" id="set-lr-secret" value="${esc(s.lrSecret || '')}" placeholder="Ваш Secret Key"/></div>
      </details>
      <details>
        <summary>Partner API (расширенные метаданные)</summary>
        <div class="form-group mt-8"><label for="set-lr-pid">Partner ID</label><input type="text" id="set-lr-pid" value="${esc(s.lrPartnerId || '')}" placeholder="Ваш Partner ID"/></div>
        <div class="form-group"><label for="set-lr-psecret">Secret Key</label><input type="password" id="set-lr-psecret" value="${esc(s.lrPartnerSecret || '')}" placeholder="Ваш Secret Key"/></div>
      </details>
      <button id="set-lr-test" class="btn-secondary mt-8">🔍 Проверить подключение</button>
      <span id="set-lr-status" class="status-text" aria-live="polite"></span>
    </div>
    ...
    <div class="settings-section">
      ...
      <span id="set-ocr-status" class="status-text" aria-live="polite"></span>
    </div>

    <div class="settings-section">
      <h3>📱 Площадки</h3>
      <div class="form-group"><label for="set-platform">Площадка по умолчанию</label>
    ...
    <div class="settings-section">
      <h3>🔔 Эффекты</h3>
      <div class="toggle-row"><span class="toggle-label">🎊 Конфетти</span><div class="toggle ${s.confetti ? 'active' : ''}" id="set-confetti" role="switch" aria-checked="${s.confetti}" tabindex="0"></div></div>
      <div class="toggle-row"><span class="toggle-label">🔊 Звуки</span><div class="toggle ${s.sound ? 'active' : ''}" id="set-sound" role="switch" aria-checked="${s.sound}" tabindex="0"></div></div>
    </div>
    ...
    <div class="settings-section">
      ...
      <p class="hint">Book Tracker Pro v3.8.3 · Трекер книг для бук-блогера · Работает оффлайн</p>
    </div>
  `;

  const bind = (id, fn) => mc.querySelector(id)?.addEventListener('click', fn);

  // 🆕 v3.8.3: toggle с keyboard support
  const bindToggle = (id, key) => {
    ...
    el.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleFn.call(el); }
    });
  };
...
// 🆕 v3.8.3: formatPrice и convertToDefault больше НЕ определяются локально.
// Они импортируются из utils.js (см. начало файла) и реэкспортируются.
...
// 🆕 v3.8.3: focus trap в оверлеях
function openOverlay(el) {
  el.classList.remove('hidden');
  document.body.style.overflow = 'hidden';
  trackOverlay(el);

  // Сохраняем текущий фокус для возврата
  ...
  // Focus trap: Tab не выходит за пределы оверлея
...
function closeOverlay(el) {
  ...
  if (el._lastFocused && el._lastFocused.focus) {
    el._lastFocused.focus();
    el._lastFocused = null;
  }
}

// 🆕 v3.8.3: focus trap helper
...
// 🆕 v3.8.3: showToast, esc, debounce больше НЕ определяются локально.
// Они импортируются из utils.js (см. начало файла).
...
function showLoading(text = 'Загрузка...') {
  ...
  loadingEl.setAttribute('role', 'alert');       // 🆕 v3.8.3
...
function fireConfetti() {
  ...
  // 🆕 v3.8.3: уважаем prefers-reduced-motion
  const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (prefersReducedMotion) {
    showToast('🎉 Поздравляем!', 'success');
    return;
  }
