...
//      — Иконки меню разного размера (NAV_ICON_SIZES)
//      — Microlink предпросмотр с маппингом полей
//      — Отчётность по контенту (reportSent/reportDate)
//      — receivedDate в карточке книги
//      — showConfirm вместо нативных confirm()
//      — Shortcuts манифеста (?action=add, ?tab=...)
//      — Обложки в поиске: __srCoverFallback
//      — Drawer logo на SVG
//
...
import { extractBookPreview, checkMicrolinkStatus, clearPreviewCache, setMicrolinkApiKey } from './microlink.js';
...
import {
attachDatePicker, attachCustomSelect, refreshCustomSelect,
showConfirm, closeAllPopups
} from './uikit.js';
// ═══════════════════════════════════════════════
//  СОСТОЯНИЕ
// ═══════════════════════════════════════════════
...
// Устанавливаем Microlink API ключ
...
// 🆕 v3.6.0: обработка shortcuts манифеста
...
// 🆕 v3.6.0: обработка shortcuts из manifest.json
...
// Сначала закрываем поповеры uikit
closeAllPopups();
...
// 🆕 v3.6.0: осмысленная шкала размеров иконок меню
// Ключевые разделы крупнее, служебные мельче
...
// 🆕 v3.6.0: фолбэк для обложек в поиске (search.js)
window.__srCoverFallback = function (img, bookId) {
const book = S.books.find(b => b.id === bookId);
if (!book) return '';
return coverPlaceholder(book, 'sr-cover ph').replace('class="sr-cover ph"', 'class="sr-cover ph" style="width:38px;height:57px"');
};
// ═══════════════════════════════════════════════
//  ВКЛАДКА: КНИГИ
// ═══════════════════════════════════════════════
...
// 🆕 v3.6.0: кастомные контролы для селектов и дат
attachCustomSelect(fb.querySelector('#bf-currency'), {});
attachCustomSelect(fb.querySelector('#bf-status'), {
renderOption: (opt) => {
const key = opt.value;
return '<span style="display:flex;align-items:center;gap:8px">' + statusIcon(key, 15) + ' ' + esc(opt.textContent) + '</span>';
},
renderTrigger: (opt) => {
const key = opt.value;
return '<span style="display:flex;align-items:center;gap:8px">' + statusIcon(key, 15) + ' ' + esc(opt.textContent) + '</span>';
...
// 🆕 v3.6.0: извлечение по ссылке → окно предпросмотра Microlink
...
// ═══════════════════════════════════════════════
/**
* Открывает окно предпросмотра извлечённых данных Microlink.
* Показывает ВСЕ найденные поля, позволяет менять назначение.
* @param {{merged, fields}} result — результат extractBookPreview
* @param {HTMLElement} fb — form-body для применения
*/
function openMicrolinkPreview(result, fb) {
const { merged, fields } = result;
// Целевые поля формы книги
const TARGETS = [
...
// Маппинг source → человекочитаемое имя
const SOURCE_LABELS = {
'json-ld': 'JSON-LD',
...
// Состояние маппинга: fieldId → targetValue
const mapping = {};
...
<h2>${icon('download', 18)} Предпросмотр данных</h2>
...
// Кастомные селекты для каждого поля
...
// Собираем значения по маппингу
const values = {};
fields.forEach(f => {
const target = mapping[f.id];
if (target && f.value) {
// Первое значение для каждого target (приоритет по порядку полей)
...
// Обновляем кастомные селекты формы
refreshCustomSelect(fb.querySelector('#bf-currency'));
refreshCustomSelect(fb.querySelector('#bf-status'));
...
// 🆕 v3.6.0: receivedDate для PR-книг
const prMeta = book.isPR && book.receivedDate
? meta('Получена', book.receivedDate + (book.receivedFrom ? ' · ' + book.receivedFrom : ''))
: '';
let html =
...
: '';
// 🆕 v3.6.0: блок отчётности
const reportSent = item.reportSent || false;
const reportDate = item.reportDate || '';
const reportBlock = `
<div class="form-group mt-16">
<label>${icon('report', 13)} Отчёт издательству</label>
<div class="cc-report" style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--bg-input);border-radius:var(--radius-sm)">
<span style="color:${reportSent ? 'var(--green)' : 'var(--red)'}">
${reportSent ? icon('checkBadge', 18) : icon('xBadge', 18)}
</span>
<span style="flex:1;font-size:.88rem;font-weight:600">
${reportSent ? 'Отправлено' : 'Не отправлено'}
</span>
${reportSent && reportDate ? `<span class="text-small text-muted">${reportDate}</span>` : ''}
<button class="btn-secondary cc-report-toggle" style="padding:6px 12px;font-size:.78rem">
${reportSent ? 'Отменить' : 'Отметить'}
</button>
</div>
</div>`;
overlay.innerHTML =
...
'</div>' + reportBlock + linkBlock + notesBlock +
...
// 🆕 v3.6.0: переключение отчёта
overlay.querySelector('.cc-report-toggle').addEventListener('click', async () => {
const newSent = !reportSent;
const updates = { reportSent: newSent };
if (newSent && !reportDate) {
updates.reportDate = new Date().toISOString().slice(0, 10);
} else if (!newSent) {
updates.reportDate = '';
}
// Обновляем контент в книге
const { updateContentInBook } = await import('./db.js');
await updateContentInBook(bookId, item.id, updates);
document.dispatchEvent(new CustomEvent('data-changed'));
close();
showToast(newSent ? '✅ Отчёт отмечен как отправленный' : 'Отчёт снят', 'success');
});
...
if (ok) { await deleteCollection(id); await refreshData(); showToast('Подборка удалена', 'info'); }
},
...
if (ok) { await deleteChallengeById(id); S.openChallenge = null; await refreshData(); showToast('Челлендж удалён', 'info'); }
},
onOpenBook: openBookDetail,
...
onDelete: async (id) => {
const ok = await showConfirm('Удалить челлендж?', { danger: true, okText: 'Удалить' });
if (ok) { await deleteChallengeById(id); await refreshData(); showToast('Удалён', 'info'); }
},
});
}
}
...
'<p class="hint">Book Tracker Pro v3.6.0 · Трекер книг для бук-блогера · Работает оффлайн</p></div>';
...
// 🆕 v3.6.0: кастомный селект для валюты
attachCustomSelect(mc.querySelector('#set-currency'), {});
...
if (mkKey) mkKey.addEventListener('change', function() {
